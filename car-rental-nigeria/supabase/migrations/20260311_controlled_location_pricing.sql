-- Controlled Location & Timing Pricing System
-- Run after 20260310_usernames_and_admin_rls.sql.
...
create table if not exists public.car_service_coverage (
  ...
  constraint car_service_coverage_rental_mode_check check (rental_mode in ('within_state', 'interstate'))
);
...
create table if not exists public.car_pricing_rates (
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  unique(car_id, state_id, zone_id, timing_package),
  constraint car_pricing_rates_timing_package_check check (timing_package in ('12h', '24h'))
);
...
create table if not exists public.quote_requests (
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint quote_requests_rental_mode_check check (rental_mode in ('within_state', 'interstate')),
  constraint quote_requests_status_check check (status in ('new', 'reviewing', 'quoted', 'closed', 'converted'))
);
...
select cars.id, service_states.id, service_zones.id, timing.package, case when timing.package = '12h' then round(cars.price_per_day * 0.75) else cars.price_per_day end, true
from public.cars
...
on conflict (car_id, state_id, zone_id, timing_package) do nothing;