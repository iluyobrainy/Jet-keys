-- Website settings migration using existing type values
-- This will use the same type values that are already in the table

-- First, let's see what type values exist
-- Then we'll use those same values for our new records

-- Clear existing data
DELETE FROM website_settings;

-- Insert data using common type values that might be allowed
-- Let's try some basic values that are commonly used
