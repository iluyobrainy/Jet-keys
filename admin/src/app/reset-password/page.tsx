...
export default function AdminResetPasswordPage() {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setMessage("Password updated successfully. Redirecting to the admin dashboard...")
      window.setTimeout(() => {
        router.replace("/")
      }, 1200)
    } catch (resetError) {
      setError(resetError instanceof Error ? resetError.message : "Unable to update the password.")
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-3xl items-center justify-center">
        <Card className="w-full max-w-xl rounded-[32px] border border-white/70 bg-white/92 shadow-[0_24px_80px_rgba(15,23,42,0.14)] backdrop-blur">
          <CardHeader className="space-y-3 px-8 pt-8">
            <Link href="/login" className="flex items-center gap-3 text-slate-900">
              <Car className="h-7 w-7 text-blue-600" />
              <span className="text-xl font-bold">Jet & Keys Admin</span>
            </Link>
            <div>
              <CardTitle className="text-3xl font-bold text-slate-950">Set a new admin password</CardTitle>
              <p className="mt-2 text-sm text-slate-600">
                Use the reset link from your email, then save the new password here.
              </p>
            </div>
          </CardHeader>
          <CardContent className="space-y-6 px-8 pb-8">
            ...
            <Button
              ...
              disabled={submitting || loading || !isAuthenticated || !isAdmin}
            >
